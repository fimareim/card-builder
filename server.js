...
// 1. Генерация текста (Pollinations.ai)
...
// 2. Генерация изображения (Pollinations.ai) – теперь с seed и правильным стилем
app.get('/api/generate-image', async (req, res) => {
    const { description, style } = req.query;
    // Формируем промпт с акцентом на стиль
    const imagePrompt = encodeURIComponent(
        ${description}, in the style of ${style}, high quality digital art
    );
    // Случайный seed, чтобы избежать кэширования
